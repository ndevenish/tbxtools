...
class Distribution(object):
    ...
    def _load_dependencies_for(self, module):
        ...
        abs_deps = set(module.config["modules_required_for_build"]) | set(
            module.config["modules_required_for_use"]
        )
        for dep_name in abs_deps:
            ...
            if self.load_module(dep_name) is None:
                raise DependencyError(
                    "Cannot find module {} (required by {})".format(
        ...
        for dep_name in module.config["optional_modules"]:
            try:
                logger.debug(
                    "Optionally loading dependency for {}: {}".format(
    ...
    """
        ...
        for dep_type in ["modules_required_for_build", "modules_required_for_use"]:
            ...
            if overlap:
                logger.debug(
                    "Module {}: Making dependenc{} {} optional".format(
                        module.name,
    ...
    def request_modules(self, names):
        ...
        for modulename in names:
            if self.load_module(modulename) is None:
                raise DependencyError(
                    "Could not find requested module {}".format(modulename)
                )
