...
class FlagViewer(set):
  def __repr__(self):
    flag_names = sorted(
        [n.name.upper() for n in self])  # Flags.resolve(self.value)
    if flag_names:
  ...
  if len(self):
    ...
    maxw = [
        max(
            ...
            for x in format_sample) for i in range(3)
    ]
    fmts = "(" + ", ".join([
...
def _cctbx_crystal_symmetry_repr(self):
  ...
  if self.space_group_info() is not None:
    parts.append("space_group_symbol='{}'".format(
